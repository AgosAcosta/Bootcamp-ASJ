...
SELECT d.nombre'Discografica', a.nombre 'Artista'
FROM artista a
...
SELECT a.nombre 'Nombre Artista', COUNT(c.id) 'Cantidad de Canciones'
FROM artista a
...
SELECT TOP 1 a.nombre 'Nombre Artista', COUNT(c.id) 'Cantidad de Canciones'
FROM artista a
JOIN album al ON al.artista_id = a.id
JOIN canciones c ON c.album_id = al.id
GROUP BY a.nombre 
...
SELECT a.nombre 'Nombre Artista', al.nombre 'Nombre Album', COUNT(c.id) 'Cantidad'
FROM artista a
JOIN album al ON al.artista_id = a.id
JOIN canciones c ON c.album_id = al.id
WHERE c.duracion < 5
...
SELECT a.nombre 'Nombre Artista', al.nombre 'Nombre Album', c.nombre 'Nombre Cancion' ,c.duracion 'Duracion'
FROM artista a
JOIN album al ON al.artista_id = a.id
JOIN canciones c ON c.album_id = al.id
WHERE c.duracion < 5;
...
SELECT a.nombre 'Nombre Artista', COUNT(c.id)
FROM artista a
JOIN album al ON al.artista_id = a.id
JOIN canciones c ON c.album_id = al.id
WHERE c.duracion > 5
...
SELECT TOP 1 COUNT(al.anio)'Cantidad de canciones', al.anio 'Año'
FROM album al 
JOIN canciones c ON c.album_id = al.id
GROUP BY al.anio;
...
SELECT TOP 5 a.nombre 'Nombre Artista', al.nombre 'Nombre Album', c.nombre 'Nombre Cancion' ,al.anio 'Año del Album',c.duracion 'Duracion'
FROM artista a
JOIN album al ON al.artista_id = a.id
JOIN canciones c ON c.album_id = al.id
...
SELECT a.nombre 'Nombre Artista', SUM(c.duracion) 'Duracion Total'
FROM artista a
JOIN album al ON al.artista_id = a.id
JOIN canciones c ON c.album_id = al.id
GROUP BY a.nombre
...
SELECT artista.nombre 'Nombre del Artista', album.nombre 'Nombre del Album'
FROM artista
    LEFT JOIN album on artista.id = album.artista_id
    WHERE (album.id NOT IN (
    SELECT album.id AS id_album
        FROM album 
            LEFT JOIN canciones on album.id = canciones.album_id
        WHERE canciones.duracion < 5
        GROUP BY album.id
));
...
SELECT TOP 3  a.nombre 'Nombre Artista' , AVG(c.duracion) 'Promedio de duracion'
FROM artista a
JOIN album al ON al.artista_id = a.id
JOIN canciones c ON c.album_id = al.id
GROUP BY a.nombre
ORDER BY 'Promedio de duracion' DESC;


--13 - Qué artistas no lanzaron un album durante la decada de 1980 y 1990?

SELECT a.nombre 'Nombre Artista'
FROM artista a
WHERE a.id NOT IN (
    SELECT DISTINCT al.artista_id
    FROM album al
    WHERE al.anio BETWEEN 1980 AND 1999
);

--14 - Duración total del album Sgt. Pepper's de los Beatles (mostrar en minutos y segundos)

SELECT FLOOR(SUM(c.duracion))  'Minutos', CEILING((SUM(c.duracion)-FLOOR(SUM(c.duracion)))*60) 'Segundos'
FROM album a
